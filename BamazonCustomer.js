//running sweet NODE Mods
var mysql = require('mysql');
var Table = require('cli-table');
var inquirer = require('inquirer');

//sql connection to bamazon DB
...
function userSelectsItem(){
  ...
  connection.query('SELECT ProductName FROM Products', function(err, res){
    ...
    inquirer.prompt([
      ...
      name: 'choices',
      type: 'checkbox',
      message: 'Select a product to add to cart? Press the space key to choose the product and press enter when you are finished.',
      choices: items
      }
    ]).then(function(user){
  ...
  inquirer.prompt([
    ...
    validate: function(str){
        if (parseInt(str) <= itemStock) {
          ...
          console.log('\nOh no! We only have ' + itemStock + ' of those in stock.');
          return false;
        }
      }
    }
...
function checkout(){
  ...
  if (shoppingCart.length != 0) {
    ...
    console.log('---------------------------------------------');
    console.log('View Your Cart. Are you ready to checkout?');
